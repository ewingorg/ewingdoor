...
import com.ewing.core.mpsdk.session.JSTicketMemoryCache;
import com.ewing.core.mpsdk.utils.HttpTool;
import com.ewing.core.mpsdk.vo.ApiResult;
import com.ewing.core.mpsdk.vo.MPAccount;
import com.ewing.core.mpsdk.vo.api.AccessToken;
...
import com.ewing.core.mpsdk.vo.api.Menu;
...
public class WechatAPIImpl implements WechatAPI {
  ...
  private static Logger logger = Logger.getLogger(WechatAPIImpl.class);
     ...
     * WechatAPI 实现方法
     * 
     * @param mpAct 微信公众号信息{@link MPAccount}
     * @return 对应的API
     */
    ...
    private synchronized void refreshJSTicket() {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Get mp[%s] JSSDK ticket failed. There try %d items.", mpAct.getMpId(),
                    i + 1);
    ...
    public List<Menu> getMenu() {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Get mp[%s] custom menu failed. There try %d items.", mpAct.getAppId(),
                    i + 1);
        }
    ...
    public boolean createMenu(Menu... menu) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Create mp[%s] custom menu failed. There try %d items.", mpAct.getAppId(),
                    i + 1);
    ...
    public boolean delMenu() {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Delete mp[%s] custom menu failed. There try %d items.", mpAct.getMpId(),
                    i + 1);
        }
    ...
    public int createGroup(String name) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Create mp[%s] group name[%s] failed. There try %d items.", mpAct.getMpId(),
                    name, i);
        }
    ...
    public int getGroup(String openId) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Get mp[%s] openId[%s] groups failed. There try %d items.", mpAct.getMpId(),
                    openId, i);
        }
    ...
    public boolean renGroups(int id, String name) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Rename mp[%s] groups[%d-%s] failed. There try %d items.", mpAct.getMpId(),
                    id, name, i);
        }
    ...
    public boolean move2Group(String openId, int groupId) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Move mp[%s] openId[%s] to groups[%d] failed. There try %d items.",
                    mpAct.getMpId(), openId, groupId, i);
        }
    ...
    public boolean delGroup(int id) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Delete mp[%s] groups[%d] failed. There try %d items.", mpAct.getMpId(), id,
                    i);
        }
    ...
    public QRTicket createQR(Object sceneId, int expireSeconds) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.infof("Create mp[%s] scene[%s] qrcode failed. There try %d items.",
                    mpAct.getMpId(), data.get("action_name"), i);
        }
    ...
    public File getQR(String ticket) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            ar = ApiResult.create((String) tmp);
            log.errorf("Download mp[%s] qrcode image failed. There try %d items.", mpAct.getMpId(),
                    i);
        }
    ...
    public boolean setIndustry(int id1, int id2) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Set mp[%s] template industry failed. There try %d items.", mpAct.getMpId(),
                    i);
        }
    ...
    @Override
    public long sendTemplateMsg(String openId, String tmlId, String topColor, String url,
            Template... tmls) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf("Send mp[%s] template message failed. There try %d items.", mpAct.getMpId(),
                    i);
        }
    ...
    public WebAuthorizationDto getWebAccessToken(String appId, String appsecret, String code) {
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            ...
            log.errorf(
                    "Get mp[%s] getWebAccessToken on web authorization failed, code[%s]. There try %d items.",
                    appId, appsecret, code, i);
        }
