...
/**
 * ResultSet封装返回beanlist
 * 
 * @author tanson lam
 * @createDate 2014年9月16日
 * 
 */
public class BeanListProcessHandler {

	private Class beanClazz;

	private void init(Class beanClazz) {
		this.beanClazz = beanClazz;
		for (Field field : beanClazz.getDeclaredFields()) {
			com.ewing.core.jdbc.annotation.Column column = field
					.getAnnotation(com.ewing.core.jdbc.annotation.Column.class);
			if (column == null)
				continue;
			String columnName = column.fieldName();
			String fieldName = field.getName();
			columnToPropertyOverrides.put(columnName, fieldName);
		}
	}
	...
	protected int[] mapColumnsToProperties(ResultSetMetaData rsmd,
		...
		for (int col = 1; col <= cols; col++) {
			...
			if (columnName.equals("GENERATED_KEY")) {
				propertyName = "id";
			}
	...
	protected Object processColumn(ResultSet rs, int index, Class<?> propType)
		...
		// fixed bug while occur wrong date time.
		try {
			rs.getObject(index);
		} catch (Exception e) {
			return null;
