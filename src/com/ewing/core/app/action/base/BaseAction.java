...
public class BaseAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {
  ...
  protected <T> T getParamJson(Class<T> clazz) throws Exception {
    ...
    try {
      if (request.getMethod().equalsIgnoreCase("POST"))
        requestJson =
            JsonUtils
                .toObject(IOUtils.toString(request.getInputStream()), RequestJson.class, clazz);
  ...
  protected void checkRequired(Object obj, String message) {
    boolean isNullOrEmpty =
        (null == obj) || (obj instanceof String && obj.toString().isEmpty())
  ...
  public <T> PageBean pageQuery(Class<T> entityClass) throws ActionException {
    ...
    PageBean pageBean =
        baseModelService.pageQuery(bulidConditionSql(), bulidOrderBySql(),
  ...
  /**
   * 获取登陆商户ID customer表id<br/>
   * 如果登陆验证设置为false,就认为是开发模式，
   * 
   * @return
      ...
      if (fValue.getClass().isArray()) {
        ...
        if (fieldType.equals("java.lang.String")) {
          ...
          if (value.length() == 10) {
            ...
            f.set(entityBean, DataFormat.stringToDate(value, DataFormat.DATETIME_FORMAT));
          }
        } else if (fieldType.equals("java.sql.Timestamp")) {
          if (value.length() == 10) {
            f.set(entityBean,
                new java.sql.Timestamp(DataFormat.stringToDate(value, DataFormat.DATE_FORMAT)
                    .getTime()));
          } else if (value.length() == 19) {
            f.set(entityBean,
                new java.sql.Timestamp(DataFormat.stringToDate(value, DataFormat.DATETIME_FORMAT)
                    .getTime()));
          }
        }
  ...
  public String bulidConditionSql() {
    ...
    for (Iterator itor = paramMap.keySet().iterator(); itor.hasNext();) {
      ...
      if (key instanceof String && key.toString().startsWith(SEARCH_ORDER_CODE) && object != null) {
        if (object.getClass().isArray()) {
          Object[] sValue = (Object[]) object;
          String value = sValue[0].toString();
          if (value.trim().isEmpty())
  ...
  public String getPaginationUrl(String paginationUrl) {
    String url =
        SysParamFactory.WEB_CONTEXT_PATH
