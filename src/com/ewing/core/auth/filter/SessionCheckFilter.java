...
public class SessionCheckFilter implements Filter{
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    String server = "http://" + req.getServerName() + (req.getServerPort() != 80 ? ":" + req.getServerPort() : "");
    if (!SystemPropertyUtils.isAuth() || ignoreCheck(requestPath)) {// 不需要登录验证或者路径不需要检查，直接跳过登陆拦截
    ...
    if (req.getSession(false) == null || null == HttpSessionUtils.getPreSessionUserDetails()) {
      ...
      String url = useApi.getWebAuthorizationCode(manager.getAppId(), URLEncoder.encode(server + context + GET_WEB_AUTH_CODE),
          UserAPI.SCOPE_SNSAPI_USERINFO, "123");
      
      //进行前端的一个重定向
      AjaxJsonpUtils.outJson(req, resp, JsonUtils.toJson(new ResponseData(true, url, ResponseType.REDIRECT)));
      return;
    }
